...
class StreamlitPlotter(DoseResponsePlotter):
    ...
    def plot_streamlit_curves(
        ...
        plot_style="default",
        ic50_vertical_color="#1f77b4",
        ic50_horizontal_color="#000000",
        dmax_observed_color="#d62728",
        dmax_predicted_color="#2ca02c",
        combined_plot=False,
        ...
        if combined_plot:
            self._create_combined_plot(
                ...
                plot_style,
                ic50_vertical_color,
                ic50_horizontal_color,
                dmax_observed_color,
                dmax_predicted_color,
                compound_colors,
            )
        else:
    ...
    def _create_combined_plot(
        ...
        for i, (compound, model_data) in enumerate(results["best_fitted_models"].items()):
            ...
            if compound_colors and compound in compound_colors:
                point_color_compound = compound_colors[compound].get(
                    "point_color", default_colors[i % len(default_colors)]
                )
                line_color_compound = compound_colors[compound].get(
    ...
    def _create_separate_plots(
        ...
        for i, (compound, model_data) in enumerate(results["best_fitted_models"].items()):
            ...
            if legend_position == "upper center":
                ax.legend(
                    fontsize=text_size - 2, loc="upper center", bbox_to_anchor=(0.5, -0.1), ncol=2
                )
            else:
            ...
            with col1:
                ic50_val = (
                    StreamlitDataFrameSerializer.convert_numpy_types(ic50)
            ...
            with col2:
                st.metric(
                    "RMSE",
                    f"{StreamlitDataFrameSerializer.convert_numpy_types(model_result['rmse']):.4f}",
                )
            with col3:
                st.metric(
                    "AIC",
                    f"{StreamlitDataFrameSerializer.convert_numpy_types(model_result['aic']):.2f}",
                )
            with col4:
                st.metric(
                    "Model",
                    StreamlitDataFrameSerializer.convert_numpy_types(model_result["model_name"]),
                )

            if i < len(results["best_fitted_models"]) - 1:
                st.divider()
...
class StreamlitDataFrameSerializer:
    ...
    def convert_numpy_types(obj):
        ...
        if isinstance(obj, pd.DataFrame):
            ...
            return pd.Series(
                [StreamlitDataFrameSerializer.convert_numpy_types(x) for x in obj.tolist()]
            )
...
def main():
    ...
    with st.sidebar:
        ...
        help_text = "Upload your own CSV/TXT file" + (
            " or load a sample file" if sample_files else ""
        )

        data_source = st.radio("Choose data source:", data_source_options, help=help_text)
        if data_source == "📁 Upload file":
            ...
            if sample_files:
                ...
                try:
                    ...
                    st.info(
                        f"Rows: {StreamlitDataFrameSerializer.convert_numpy_types(len(data))}, Compounds: {StreamlitDataFrameSerializer.convert_numpy_types(data['Compound'].nunique())}"
                    )
            ...
            st.error(
                "❌ No data source available. Please upload a file or ensure sample files are available."
            )
            st.stop()
        ...
        log_transformed = st.checkbox(
            "Concentration data is already log-transformed",
            value=False,
        ...
        plot_style = st.selectbox(
            ...
            style_options,
            index=0,
            help="Matplotlib style for overall plot appearance",
        )
        ...
        with col_grid2:
            ...
            legend_position = st.selectbox(
                "Legend position",
                ["upper center", "upper right", "lower right", "lower left", "center"],
                index=0,
            )
        ...
        if num_compounds > 1:
            ...
            st.write(
                f"*Found {num_compounds} compounds: {', '.join(unique_compounds[:3])}{'...' if num_compounds > 3 else ''}*"
            )

            custom_compound_colors = st.checkbox(
                "Customize colors for each compound",
            ...
            if custom_compound_colors:
                ...
                st.info(
                    "💡 **Tip**: Enable 'Customize colors for each compound' above to distinguish compounds better in combined plots!"
                )
    ...
    with col1:
        ...
        st.write(
            f"**Unique compounds:** {StreamlitDataFrameSerializer.convert_numpy_types(data[compound_col].nunique())}"
        )
        try:
            conc_min = StreamlitDataFrameSerializer.convert_numpy_types(
                data[concentration_col].min()
            )
            conc_max = StreamlitDataFrameSerializer.convert_numpy_types(
                data[concentration_col].max()
            )
        ...
        if len(compounds_str) > 5:
            displayed_compounds = (
                ", ".join(compounds_str[:5]) + f" (and {len(compounds_str)-5} more)"
            )
        else:
    ...
    with col2:
        if st.button("🚀 Run Dose-Response Analysis", type="primary", use_container_width=True):
            if (
                compound_col == concentration_col
                or compound_col == response_col
                or concentration_col == response_col
            ):
                st.error(
                    "❌ Please select different columns for compound, concentration, and response."
                )
                st.stop()
            ...
            try:
                ...
                if concentration_col in data_for_analysis.columns:
                    data_for_analysis[concentration_col] = pd.to_numeric(
                        data_for_analysis[concentration_col], errors="coerce"
                    ).astype("float64")
                if response_col in data_for_analysis.columns:
                    data_for_analysis[response_col] = pd.to_numeric(
                        data_for_analysis[response_col], errors="coerce"
                    ).astype("float64")
                if compound_col in data_for_analysis.columns:
                    data_for_analysis[compound_col] = data_for_analysis[compound_col].astype(
                        "string"
                    )

                data_for_analysis = data_for_analysis.dropna(
                    subset=[compound_col, concentration_col, response_col]
                )
    ...
    if "results" in st.session_state:
        ...
        with tab2:
            ...
            safe_summary_table = StreamlitDataFrameSerializer.convert_numpy_types(
                st.session_state.results["summary_table"]
            )
            ...
            with col1:
                st.metric(
                    "Total Models Fitted",
                    StreamlitDataFrameSerializer.convert_numpy_types(
                        len(st.session_state.results["summary_table"])
                    ),
                )
            with col2:
                avg_rmse = StreamlitDataFrameSerializer.convert_numpy_types(
                    st.session_state.results["summary_table"]["RMSE"].mean()
                )
                st.metric("Average RMSE", f"{avg_rmse:.4f}")
            with col3:
                successful_fits = StreamlitDataFrameSerializer.convert_numpy_types(
                    len(st.session_state.results["best_models"])
                )
        ...
        with tab3:
            ...
            safe_best_models = StreamlitDataFrameSerializer.convert_numpy_types(
                st.session_state.results["best_models"]
            )
        ...
        with tab4:
            ...
            with col1:
                safe_summary = StreamlitDataFrameSerializer.convert_numpy_types(
                    st.session_state.results["summary_table"]
                )
            ...
            with col2:
                safe_best_models_dl = StreamlitDataFrameSerializer.convert_numpy_types(
                    st.session_state.results["best_models"]
                )
            ...
            info_data = {
                ...
                "Value": [
                    ...
                    StreamlitDataFrameSerializer.convert_numpy_types(
                        str(len(st.session_state.results["best_models"]))
                    ),
                    StreamlitDataFrameSerializer.convert_numpy_types(
                        str(len(st.session_state.data_for_analysis))
                    ),
